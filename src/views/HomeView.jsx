...
//           <div className="text-[9px] text-slate-500 tracking-wide">{item.label}</div>
//         </div>
//       ))}
//     </div>
//   );
...
//                 The <span className="text-white font-medium">International Federation of Esports (IFES)</span> is a global governing platform committed to building a
//                 <span className="text-white font-medium"> structured, inclusive, and future-ready esports ecosystem</span>.
//               </p>

//               <p className="text-sm text-slate-400 leading-relaxed max-w-lg">
//                 From international tournaments to policy development, IFES is transforming esports into a
//                 <span className="text-white font-medium"> recognized, respected, and career-driven global sport</span>.
//               </p>

//               {/* CTA */}
//               <div className="pt-2">
//                 <button className="bg-gradient-to-r from-purple-600 to-purple-500 px-5 py-2.5 text-sm font-medium text-white rounded-md hover:brightness-110 transition">
//                   Join the Movement
//                 </button>
//               </div>
...
//                 <div className="relative rounded-2xl p-7 bg-slate-900/80 backdrop-blur-md border border-white/5 overflow-hidden">
...
//                       NEXT EVENT
//                     </span>
//                     <span className="text-[10px] text-slate-500">07-08 August 2026</span>
//                   </div>
...
//                   <div className="flex justify-between items-center text-sm bg-black/40 border border-cyan-500/20 rounded-lg px-4 py-2.5 mt-4">
//                     <span className="text-slate-400">Prize Pool</span>
//                     <span className="text-cyan-400 font-semibold">$2.5M</span>
//                   </div>

//                   <button className="w-full mt-5 py-2.5 rounded-lg text-sm font-medium text-white bg-gradient-to-r from-cyan-600 to-purple-600 hover:brightness-110 transition">
//                     REGISTER ESCOM
//                   </button>
...
//               </svg>
//               <h2 className="text-2xl md:text-3xl font-extrabold inline-block ml-2 tracking-tight">ESCOM<span className="text-cyan-400">2.O</span></h2>
...
//                   LIVE NOW · 2nd ROUND
//                 </div>
//                 <p className="text-xs text-slate-300 mt-2">The eSports Community and Industry Summit (ESCOM) is a premier event that unites industry leaders, gamers, and enthusiasts to explore the latest trends, innovations, and opportunities in eSports.</p>
...
//             {[
//               { key: 'challenges', label: 'Countries', fallback: '50+', suffix: '' },
//               { key: 'teams', label: 'Partnerships', fallback: '140+', suffix: '' },
//               { key: 'club', label: 'Active Clubs', fallback: '3800+', suffix: '' },
//               { key: 'member', label: 'Community Reach', fallback: '1200,000+ ', suffix: 'M', divider: 1000000 },
//               { key: 'viewership', label: 'Global viewership', fallback: '200,000,000+', suffix: 'M', divider: 1000000 },
//             ].map((stat) => {
...
//                   <p className="text-xs md:text-sm font-medium">Uganda joins IFES as 43rd member federation</p>
//                 </div>
...
const HomeView = ({ setView, siteConfig, newsItems = [], newsLoading, newsError, locationCode: locationCodeProp }) => {
  ...
  useEffect(() => {
    const fetchBreakingNews = async () => {
      ...
      try {
        const response = await axios.get('https://app.aicra.org/api/escomapi/breaking-escom-news-feed.php');
        if (response.data && response.data.status === true && response.data.posts) {
          // Transform breaking news posts
          const formattedBreakingNews = response.data.posts.slice(0, 5).map(post => ({
            id: post.id,
            title: post.title,
            date: post.date,
            link: post.link,
  ...
  const handleNewsClick = async (slug, title) => {
    ...
    try {
      const response = await axios.get(`https://app.aicra.org/api/escomapi/escom-news-feed-single.php?slug=${slug}`);
              ...
              <div className="prose prose-invert max-w-none">
                <div 
                  dangerouslySetInnerHTML={{ 
                    __html: selectedNews.content || selectedNews.body || '<p>Content not available</p>'
                  }} 
              ...
              <div className="mt-3 space-y-3 max-h-64 overflow-y-auto pr-1 custom-scroll">
                {breakingNewsLoading ? (
                  <div className="text-center py-4">
                    ...
                    <p className="text-xs text-slate-400 mt-2">Loading news...</p>
                  </div>
                ) : breakingNewsError ? (
                  <div className="text-center py-4">
