...
export const SquadManager = ({ setPage, user: propUser, initialEditSquad, onInitialEditSquadConsumed }) => {
  ...
  const [zrcRegion, setZrcRegion] = useState(''); // Track selected ZRC region (south, east, west, north)
  ...
  useEffect(() => {
    ...
    const applySquadData = (squad) => {
      ...
      if (eventType) {
        ...
        // Prefill ZRC region when available on squad payload
        if (eventType.toUpperCase() === 'ZRC') {
          const region =
            ...
            squad.region ||
            squad.event?.region ||
            squad.event?.zrcRegion ||
            squad.event?.zrc_region;
          if (region) {
            setZrcRegion(String(region).toLowerCase());
          }
        }
      }
      ...
      // Normalize bot id (can be an id or object)
      const rawBotId = lineup.bot_id ?? squad.lineup?.bot_id;
      const botId =
    ...
    fetchTeamDetails(editingSquad.clubId, editingSquad.teamId)
      ...
      .catch(() => {});
    return () => { cancelled = true; };
  }, [editingSquad?.clubId, editingSquad?.teamId, editingSquad?.squadData, clubs, competitions, clubMembers, bots]);
   ...
   * { club_id, category, teamName, event_id, competition_id, lineup: { bot_id, captain_id, members }, entry_fee }
  ...
  const buildSquadAddPayload = () => {
    ...
    const competition_id = String(getCompetitionId(competitionType) ?? '');
    const event_id =
      String(
        selectedComp?.event?._id ?? selectedComp?.event_id ?? getCompetitionId(competitionType) ?? ''
      ) || competition_id;
    const category = String(
      selectedCategoryValue || selectedComp?.category || 'Senior'
    );
    ...
    const bot_id = String(
      selectedBot?.bot_id ?? selectedBot?._id ?? selectedBot?.id ?? ''
    );
    ...
    const entry_fee =
      Number(selectedComp?.entry_fee ?? selectedComp?.entryFee ?? 500) || 500;
  ...
  const createTeam = () => {
    ...
    const event_id =
      String(
        selectedComp?.event?._id ??
          selectedComp?.event_id ??
          competitionId ??
          ''
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-col sm:flex-row sm:items-end gap-4 mb-3">
                ...
                <div className="flex-col flex items-start gap-2">
                 ...
                 {/* ZRC Region Selectbox - Only show when ZRC is selected */}
          {selectedEventType && selectedEventType.toUpperCase() === 'ZRC' && (
            <div className="mb-4  bg-purple-900/20 border border-purple-500/50 rounded-xl shadow-lg shadow-purple-900/20">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <div className="relative flex-1 min-w-[250px]">
                  ...
                  >
                    <option value="" className="bg-slate-800 text-slate-300">Select a region</option>
                    <option value="south" className="bg-slate-800 text-white">South</option>
                    <option value="east" className="bg-slate-800 text-white">East</option>
                    <option value="west" className="bg-slate-800 text-white">West</option>
                    <option value="north" className="bg-slate-800 text-white">North</option>
                  </select>
                  <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                ...
                {zrcRegion && (
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-green-900/30 border border-green-500/30 rounded-lg">
                    <div className="w-2 h-2 rounded-full bg-green-400 animate-pulse"></div>
                    <p className="text-green-400 text-xs font-semibold capitalize">{zrcRegion}</p>
                  </div>
                )}
              </div>
            </div>
                        ...
                        {filteredClubMembers.map((member) => {
                          ...
                          return (
                            <div
                              ...
                              className={`rounded-lg p-4 transition-all group cursor-pointer select-none ${
                                isPilot
