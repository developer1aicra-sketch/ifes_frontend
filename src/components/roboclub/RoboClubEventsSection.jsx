...
function EventHeroCaption({ ev }) {
  ...
  if (fp?.name) {
    const heroKicker = fp.theme || 'Famous place';
    return (
      <div className="absolute bottom-0 left-0 right-0 z-20 px-4 pb-3 pt-12 bg-gradient-to-t from-[#070b14] via-[#070b14]/88 to-transparent border-t border-white/5">
        <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-amber-300/90 mb-1">
          {heroKicker}
        </p>
        <p className="text-base font-black text-white leading-snug drop-shadow-sm">{fp.name}</p>
...
const RoboClubEventsSection = () => {
  ...
  // ✅ IMPORTANT: backgroundColor via inline style overrides any Tailwind bg-* class
  const sectionStyle = {
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8 mb-12">
          <div>
            ...
            <h2 className="text-4xl font-black text-white mt-2 mb-3 leading-tight">
              Competitions &amp;{' '}
              <span className="text-cyan-400">Events</span>
            </h2>
            <p className="max-w-xl text-sm leading-relaxed text-slate-400 italic">
              Participate in exciting robotics competitions and events, showcase innovations,
              compete globally, and gain recognition through TechnoXian platforms.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {FILTERS.map((f) => (
              <button
                ...
                style={filter !== f.id ? { backgroundColor: '#0a1628' } : {}}
              >
                {f.label}
              </button>
            ))}
        ...
        {filtered.length === 0 ? (
          <div
            className="text-slate-500 text-center py-16 border border-dashed border-slate-800 rounded-2xl"
          ...
          <div
            className="relative rounded-3xl border border-slate-800 backdrop-blur-sm p-5 md:p-8 overflow-hidden"
            style={containerStyle}
          >
            <div className="absolute -top-24 -right-24 w-80 h-80 rounded-full bg-cyan-600/10 blur-[120px] pointer-events-none" />
            <div className="absolute -bottom-24 -left-24 w-80 h-80 rounded-full bg-violet-600/10 blur-[120px] pointer-events-none" />
